...
module Material =
 ...
 // let mkSample sampler =  mkMultiJitteredSampler sampler 83
  ...
  let mkSpecularTexture e = mkMatTexture(mkPhongMaterial cwhite 0.0 cwhite 0.0 cwhite 1.0 e)
  let scenePhongSpecularPoint sampler () =
      renderScenePointF (mkMatteTexture 0.2 0.8 (mkMonochrome 0.8)) (mkSpecularTexture 4)
          (mkSpecularTexture 1000) (mkSpecularTexture 100) (mkSpecularTexture 10) 
          (mkSpecularTexture 100) (ambientLight ()) [mkLight (mkPoint 200.0 25.0 75.0) cwhite 1.5] sampler 1.0

  let scenePhongSpecularDirectional sampler () =
      renderScenePointF (mkMatteTexture 0.2 0.8 (mkMonochrome 0.8)) (mkSpecularTexture 4)
          (mkSpecularTexture 1000) (mkSpecularTexture 100) (mkSpecularTexture 10) 
          (mkSpecularTexture 100) (ambientLight ()) [mkDirectionalLight (mkVector 200.0 25.0 75.0) cwhite 1.5] sampler 1.0
  ...
  let renderWithSampler (gname : string) (sampler : unit -> sampler) : Target list = 
    List.map (fun (test, name) -> mkTarget gname ((fun _ -> test (sampler ()) ()), name))
      [(scene, "AmbientLight");
       ...
       (scenePhongSpecularPoint, "phongSpecularPoint");
       (scenePhongSpecularDirectional, "phongSpecularDirectional");
