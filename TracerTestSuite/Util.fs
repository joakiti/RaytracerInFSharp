...
module Util =
  ...
  let private timings2 = result_path + "/runtimeTest.csv"
  ...
  let init2 () = 
    Directory.CreateDirectory result_path |> ignore
    timings_wr <- new StreamWriter(timings2, false)
    
  let mutable acc = 0.
  let mutable acr = 0.
  let mutable count = 0.
  let write (i, cut, s, overlap) = 
      timings_wr.WriteLine("KDTREE TEST " + string i)
      timings_wr.WriteLine("CUTOFF[ " + string cut + " ] -- MIN_EMPTY_SPACE[ " + s + " ] -- MAX_OVERLAP[ " + string overlap + " ]")
      timings_wr.WriteLine("test name, construction, rendering, total")
  let reset () = 
      let avgC, avgR, avgT = acc/count , acr/count , (acc+acr)/count
      timings_wr.WriteLine("AVG CONSTRUCT[ " + string avgC + " ] -- AVG RENDERING[ " + string avgR + " ] -- AVG TOTAL[ " + string avgT + " ]")
      timings_wr.WriteLine(" ")
      acc <- 0.
      acr <- 0.
      count <- 0.
